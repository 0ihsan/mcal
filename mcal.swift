...
Manage your today in macOS/Calendar.app
 ...
 mcal \u{001B}[35mmove\u{001B}[0m [ \u{001B}[34m<id>\u{001B}[0m \u{001B}[33m<calendar> \u{001B}[0m] - move event to different calendar
...
let fileURL = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
    .appendingPathComponent("mcal_config.conf")
var calendarTitles : [String]? = [""];
do {
    // Načtení dat ze souboru
    let data = try Data(contentsOf: fileURL)
    
    // Deserializace JSON dat
    let json = try JSONSerialization.jsonObject(with: data, options: [])
    
    // Zpracování JSON objektu
    if let jsonObject = json as? [String: Any], let calendarArray = jsonObject["calendars"] as? [String] {
        print("Loaded calendars: \(calendarArray)")
         calendarTitles = calendarArray
    } else {
        print("Failed to parse JSON.")
    }
} catch {
    print("Failed to load calendar configuration file: \(error.localizedDescription)")
}
//let calendarTitles = ["work", "done"]

let c = calendars.filter { calendarTitles!.contains($0.title.lowercased()) }
//var m=Calendar.current.startOfDay(for:t);
let m=Calendar.current.date(from: Calendar.current.dateComponents([.year], from: Date()))! //work with event for all year
